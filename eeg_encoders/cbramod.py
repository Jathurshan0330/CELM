...
Paper: Wang, J., Zhao, S., Luo, Z., Zhou, Y., Jiang, H., Li, S., ... & Pan, G. (2024). Cbramod: A criss-cross brain foundation model for eeg decoding. arXiv preprint arXiv:2412.07236.
"""
...
class CBraMod_Wrapper(nn.Module):
    def __init__(self, 
                 device,
                 cls_only = False):
        super().__init__()
        ...
        print('Loading CBraMod model')
        self.cbramod = CBraMod(in_dim=200, out_dim=200, d_model=200, dim_feedforward=800, seq_len=30, n_layer=12,
                    nhead=8)
        self.cbramod.load_state_dict(torch.load('./eeg_encoders/pretrained_checkpoints/cbramod_pretrained_weights.pth',
                                                      map_location=self.device))
        self.cbramod = self.cbramod.to(self.device)
        self.cbramod.eval()
    ...
    def forward(self, x):
        B,C,T = x.shape
        x = rearrange(x, 'B C (S T) -> B C S T',T = 200)
        eeg_embed = self.cbramod(x)

        if self.cls_only:
            eeg_embed = rearrange(eeg_embed,'B C S E -> B E C S')
            eeg_embed = self.pool(eeg_embed).squeeze()#.unsqueeze(1)
        else:
            eeg_embed = rearrange(eeg_embed,'B C S E -> B (C S) E')
        return eeg_embed
