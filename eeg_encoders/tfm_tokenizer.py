"""
Code and Model Credits:
Code adapted from: https://github.com/Jathurshan0330/TFM-Tokenizer
Paper: Pradeepkumar, J., Piao, X., Chen, Z., & Sun, J. (2025). Tokenizing Single-Channel EEG with Time-Frequency Motif Learning. arXiv preprint arXiv:2502.16060.
"""
...
def get_stft_torch_tfm_tokenizer(X, resampling_rate = 200):
    B,C,T = X.shape
...
class TFM_TOKEN_Classifier(nn.Module):
    ...
    def get_eeg_embedding(self, x,num_ch = 16,cls_only = False):
        x = self.eeg_token_embedding(x)
...
class TFM_Tokenizer(nn.Module):
    def __init__(self, 
        ...
        # Initialing and loading tfm tokenizer
        print('Loading TFM-TOKEN2 2x2x8 tokenizer model')
        self.tfm_tokenizer = get_tfm_tokenizer_2x2x8(code_book_size=8192,
                                                     emb_size=64)
        self.tfm_tokenizer.load_state_dict(torch.load('./eeg_encoders/pretrained_checkpoints/tfm_tokenizer_last.pth',
                                                      map_location=self.device))
        self.tfm_tokenizer.to(self.device)
        self.tfm_tokenizer.eval()
        
        # to filter 16 channels to match the pretrained model
        self.ch_ind = [0,1,2,3,5,6,7,8,10,11,13,14,16,17,18,19]
        
        # Initialing and loading downstream transformer
        print('Loading downstream transformer model')
        self.downstream_transformer = get_tfm_token_classifier_64x4(n_classes=8192,
                                                                    code_book_size=8192, 
                                                                    emb_size=64)
        
        self.downstream_transformer.load_state_dict(torch.load('./eeg_encoders/pretrained_checkpoints/tfm_encoder_mtp_last.pth',
                                                      map_location=self.device))
        self.downstream_transformer.to(self.device)
        self.downstream_transformer.eval()
    ...
    def forward(self, x):
        x_temporal = x 
        x_temporal = x_temporal[:,self.ch_ind]
        B,C,T = x_temporal.shape

        # apply STFT
        x = get_stft_torch_tfm_tokenizer(x_temporal, resampling_rate = 200)
        x = rearrange(x,'B C F T -> (B C) F T').to(x_temporal.device)
        x_temporal = rearrange(x_temporal,'B C T -> (B C) T')

        with torch.no_grad():
            _,x_tokens,_ = self.tfm_tokenizer.tokenize(x,x_temporal)
            x_tokens = rearrange(x_tokens,'(B C) T -> B C T', C=C)
            
        eeg_embed = self.downstream_transformer.get_eeg_embedding(x_tokens,num_ch = C,cls_only= self.cls_only)
        eeg_embed= eeg_embed.squeeze()
        return eeg_embed