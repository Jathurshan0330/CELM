...
Code adapted from: https://github.com/935963004/LaBraM
Paper: Jiang, W. B., Zhao, L. M., & Lu, B. L. (2024). Large brain model for learning generic representations with tremendous EEG data in BCI. arXiv preprint arXiv:2405.18765.
"""
...
class LaBraM(nn.Module):
    def __init__(self, 
        ...
        self.in_ch = get_input_chans(ch_names)
        # print(self.in_ch)
        # Initialing and loading tfm tokenizer
        print('Loading LaBraM-Base model')
        self.labram = labram_base_patch200_200(init_values = 0,
                                               num_classes = 8192)
        checkpoint = torch.load('./eeg_encoders/pretrained_checkpoints/labram-base.pth', map_location=self.device,weights_only =False)
        ...
        load_state_dict(self.labram, checkpoint_model, prefix='')
       
        self.labram.to(self.device)
        self.labram.eval()
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            eeg_embed = self.labram.forward_features(x, 
                                                    input_chans=self.in_ch, 
                                                    return_all_tokens=True)
            
        if self.cls_only:
            eeg_embed = eeg_embed[:,0]#.unsqueeze(1)
        else:
            eeg_embed = eeg_embed[:,1:]
        return eeg_embed